@extends('layouts.app')
    @section('content')
        <section id="why-us" class="why-us">
            <div class="container-fluid">

                <header class="section-header"  >
                    <strong style="text-align: center;"> 
                        <h3>Nowadays, there is a bunch of proofreading companies. So it is normal to wonder why choose <label for="" style="color: darkred; font-weight: bolder;">S</label><label for="" style="color: black;font-weight: bolder;">ci</label><label for="" style="color: darkred;font-weight: bolder;">P</label><label for="" style="color: black;font-weight: bolder;">ub</label>? However, <label for="" style="color: darkred; font-weight: bolder;">S</label><label for="" style="color: black;font-weight: bolder;">ci</label><label for="" style="color: darkred;font-weight: bolder;">P</label><label for="" style="color: black;font-weight: bolder;">ub</label> is more than a simple manuscript editing company:
                        </h3>
                    </strong>
                </header>

                <div class="row pt-5" style="display: flex;">

                    <div class="col-lg-6">
                        <div class="why-us-content">
                            <div class="features wow bounceInUp clearfix p-3">
                                <div class="box p-2">
                                    <div class="icon" style="background: #fceef3;"><i class="fas fa-seedling" style="color: #ff689b;"></i></div>
                                    <h4 style="color: darkred;font-weight: bold;font-size: 22px;">Our focus is Plant Sciences</h4>
                                    <p style="text-align: justify;font-size: 18px;color: black; ">We dedicate our services to a specific research field in order to provide the best service to our customers. All solicitations beyond Plant Sciences field are systematically rejected.</p>
                                </div>
                            </div>
                            <div class="features wow bounceInUp clearfix p-3">
                                <div class="box p-2">
                                    <div class="icon" style="background: #fceef3;"><i class="fas fa-handshake" style="color: #ffb774;"></i></div>
                                    <h4 style="color: darkred;font-weight: bold;font-size: 22px;">Customer satisfaction is our first goal</h4>
                                    <p style="text-align: justify;font-size: 18px;color: black; ">Yes, it is true that companies work for living, but at <br> <label for="" style="color: darkred; font-weight: bolder;">S</label><label for="" style="color: black;font-weight: bolder;">ci</label><label for="" style="color: darkred;font-weight: bolder;">P</label><label for="" style="color: black;font-weight: bolder;">ub</label> above all, we work to satisfy our customers in order to retain them. We work in symbiosis with our customers while doing our best to offer the highest possible quality of service. As we know, quality begets trust. And it is the trust of the customers that our company wants to earn through its unmatched services.</p>
                                </div>
                            </div>

                            <div class="features wow bounceInUp clearfix p-3">
                                <div class="box p-2">
                                    <div class="icon" style="background: #fceef3;"><i class="fas fa-sitemap" style="color: #ff4884;"></i></div>
                                        <h4 style="color: darkred;font-weight: bold;font-size: 22px;"><label for="" style="color: darkred; font-weight: bolder;">S</label><label for="" style="color: black;font-weight: bolder;">ci</label><label for="" style="color: darkred;font-weight: bolder;">P</label><label for="" style="color: black;font-weight: bolder;">ub</label> is rich pool of scientific expertise</h4>
                                        <p style="text-align: justify;font-size: 18px;color: black; ">Our multidisciplinary and qualified team is a great strength of our company. The team is made up of experienced writers and talented researchers as well as members of prestigious publishing houses. Our team understands the difficulties associated with publication and that is why we help you taking away barriers throughout the process.</p>
                                
                                </div>

                            </div>
                            <div class="features wow bounceInUp clearfix p-3">
                                <div class="box p-2">
                                    <div class="icon" style=""><i class="far fa-gem" style="color: #ff5774;"></i></div>
                                    <h4 style="color: darkred;font-weight: bold;font-size: 22px;">Best quality and price ratio </h4>
                                    <p style="text-align: justify;font-size: 18px;color: black; ">The prices of our services are the lowest on the market and we offer bulk discounts.</p>
                            
                                </div>

                            </div>
                        </div>
                    </div>

                    <div class="col-lg-6">
                        <div class="why-us-content">
                            <div class="features wow bounceInUp clearfix p-3">
                                <div class="box p-2">
                                    <div class="icon" style=""><i class="fas fa-tachometer-alt" style="color: #ff3774;"></i></div>
                                    <h4 style="color: darkred;font-weight: bold;font-size: 22px;">Efficiency and rapidity of our services</h4>
                                    <p style="text-align: justify;font-size: 18px;color: black; ">We check accurately your manuscript, enhance the production and get it ready for publication within the shortest duration.</p>
                            
                                </div>

                            </div>
                            <div class="features wow bounceInUp clearfix p-3">
                               <div class="box p-2">
                                    <div class="icon" style=""><i class="fas fa-headset" style="color: #ff1774;"></i></div>
                                    <h4 style="color: darkred;font-weight: bold;font-size: 22px;">Support from start to publication of your project</h4>
                                    <p style="text-align: justify;font-size: 18px;color: black; ">We provide a dedicated support to our customers and partners. We assist you from the experiment set-up to the publication of your findings.</p>
                            
                                </div>

                            </div>
                            <div class="features wow bounceInUp clearfix p-3">
                                <div class="box p-2">
                                    <div class="icon" style=""><i class="fas fa-language" style="color: #ff2774;"></i></div>
                                    <h4 style="color: darkred;font-weight: bold;font-size: 22px;">Second round of proofreading for free</h4>
                                    <p style="text-align: justify;font-size: 18px;color: black; ">We always offer two rounds of editing and proofreading absolutely free of cost. 99.9 % is not enough for us. Our mission is to reach 100% of accuracy and flawlessness.</p>
                            
                                </div>
                            
                            </div>
                            <div class="features wow bounceInUp clearfix p-3">
                                <div class="box p-2">
                                    <div class="icon" style="background: #fceef3;"><i class="fas fa-key" style="color: #ffb774;"></i></div>
                                    <h4 style="color: darkred;font-weight: bold;font-size: 22px;">Full confidentiality</h4>
                                    <p style="text-align: justify;font-size: 18px;color: black; ">With SciPub your intellectual properties are safe. You will never see your data used anywhere. We will also deliver you a signed certificate from our Chief Editor.</p>
                            
                                </div>
                            
                            </div>
                            <div class="features wow bounceInUp clearfix p-3">
                                <div class="box p-2">
                                    <div class="icon" style="background: #fceef3;"><i class="fas fa-trophy" style="color: #ffb774;"></i></div>
                                    <h4 style="color: darkred;font-weight: bold;font-size: 22px;">Long experience </h4>
                                    <p style="text-align: justify;font-size: 18px;color: black; ">We have extensive experience working with worldwide Academics and top scientific publishers over 10 years.</p>
                            
                                </div>
                            </div>
                        </div>

                    </div>

                </div>

                <div class="section cl">
                    <div class="container">
                        <div class="row justify-content-center">
                            <div class="col-md-2 col-sm-4 m-2 col-xs-12 text-center" style="font-size:12px;">
                                <span data-scroll class="stat_count " style="color:#413e66;font-size:22px;font-weight:bold;">2106</span>
                                <p class="" style="color:black;font-size:20px;font-weight:bold;">Clients</p>
                            </div><!-- end col -->

                            <div class="col-md-2 col-sm-4 m-2 col-xs-12  text-center" style="font-size:12px;">
                                <span data-scroll class="stat_count " style="color:#413e66;font-size:22px;font-weight:bold;">865</span>
                                <p class="" style="color:black;font-size:20px;font-weight:bold;">Projects</p>
                            </div><!-- end col -->

                            <div class="col-md-2 col-sm-4 m-2 col-xs-12 text-center" style="font-size:12px;">
                                <span data-scroll class="stat_count " style="color:#413e66;font-size:22px;font-weight:bold;">579</span>
                                <p class="" style="color:black;font-size:20px;font-weight:bold;">Published papers</p>
                            </div><!-- end col -->

                            <div class="col-md-3 col-sm-4 m-2 col-xs-12  text-center" style="font-size:12px;">
                                <span data-scroll class="stat_count" style="color:#413e66;font-size:22px;font-weight:bold;">72</span>
                                <p class="" style="color:black;font-size:20px;font-weight:bold;">Scientific journals</p>
                            </div><!-- end col -->

                            <div class="col-md-3 col-sm-4 m-2 col-xs-12  text-center" style="font-size:12px;">
                                <span data-scroll class="stat_count " style="color:#413e66;font-size:22px;font-weight:bold;">3110</span>
                                <p class="" style="color:black;font-size:20px;font-weight:bold;">Total impact factor</p>
                            </div><!-- end col -->
                        </div><!-- end row -->
                    </div><!-- end container -->
                </div><!-- end section -->


        </section>
        <!-- End Why Us Section -->
       @endsection